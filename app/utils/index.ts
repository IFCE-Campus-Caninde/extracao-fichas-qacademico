...
export async function getRectsFromImage(
  imageData: string,
  rects: Rect[],
  out_mime_type: string = 'image/jpeg'
): Promise<Blob[]> {
  if (!import.meta.client) {
    return []
  }
...
export async function processPages(
  pages: PDFPages,
  cutRects: CutRects,
  progress_state?: Ref<ProgressState>
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    const page = pages[i]!
    const all_cropped = await getRectsFromImage(
      page,
      [...cutRects.images, ...cutRects.texts],
      'image/jpeg'
    )
