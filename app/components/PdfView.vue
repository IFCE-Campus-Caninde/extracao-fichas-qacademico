...
const cut_config = defineModel<CutConfig>('cutConfig', {
  required: true
})
...
const drawCuts = () => {
  ...
  const nextX =
    cut_config.value.startX + cut_config.value.width + cut_config.value.gapX
  ctx.beginPath()
  ...
  const nextY =
    cut_config.value.startY + cut_config.value.height + cut_config.value.gapY
  ctx.beginPath()
  ...
  while (true) {
    ...
    if (i > 100) {
      console.warn(
        'Loop exceeded 100 iterations, breaking to prevent infinite loop'
      )
      break
    }
  }
